...
object Entities {
  ...
  def main(args: Array[String]):Unit = {
    ...
    val conf=new SparkConf().setMaster("local[2]").setAppName("Entities")
...
**/
  ...
  def tokensToEntities(result: Array[(String, String)], tokens: List[CoreLabel], n: Int): Array[(String, String)] = {
    if (n==tokens.size()) { result }
      ...
      val token=tokens.get(n)
      val entity=token.get(classOf[CoreAnnotations.NamedEntityTagAnnotation])
      tokensToEntities( result:+(token.toString.replaceFirst("-\\d+$", ""), tokens.get(n).get(classOf[CoreAnnotations.NamedEntityTagAnnotation])), tokens, n+1 )
    }
  }
/*
  def tokensToString(result: String, it: util.Iterator[CoreLabel]): String = {
    if (it.hasNext) { result }
    else { result+"_" }
  }
*/
  @tailrec
  def getMentions(result: Array[(String, String)], mentions: List[CoreMap], n: Int): Array[(String, String)] = {
    if (n==mentions.size()) { result }
    else {
      val mention = mentions.get(n)
      val entityType = mention.get(classOf[CoreAnnotations.NamedEntityTagAnnotation])
      val tokens = mention.get(classOf[CoreAnnotations.TokensAnnotation])
      val entity = tokens.toString
        .replaceAll("\\,", "")
        .replaceAll("\\[", "")
        .replaceAll("\\]", "")
        .replaceAll("-\\d+", "")
        .replaceAll(" ", "_")
      getMentions(result:+(entity, entityType), mentions, n+1)
    }
    //println("entityType= "+entityType+" tokens= "+tokens+" entity= "+entity)
    //Array(("hat", "cat"))
  }
  ...
  def getEntities(text: String): Array[(String, String)] = {
    ...
    val mentions=sentence.get(classOf[CoreAnnotations.MentionsAnnotation])
    val result: Array[(String, String)]=Array()
    getMentions(result, mentions, 0)
    //val result: Array[(String, String)]=Array()
  ...
  @tailrec
  def sentencesToArrayCoreMap(result: Array[CoreMap], sentences: List[CoreMap], n: Int): Array[CoreMap] = {
    if (n==sentences.size()) { result }
    else { sentencesToArrayCoreMap(sentences.get(n)+:result, sentences, n+1) }
  }

  def getSentencesCoreMap(text: String): Array[CoreMap] = {
    val props = new Properties()
    props.setProperty("ner.useSUTime", "false")
    props.setProperty("annotators", "tokenize,ssplit,pos,lemma,ner,entitymentions,depparse,natlog,openie")
    val pipeline = new StanfordCoreNLP(props)
    val doc=new Annotation(text)
    pipeline.annotate(doc)
    val sentences=doc.get(classOf[CoreAnnotations.SentencesAnnotation])
    val result: Array[CoreMap]=Array()
    sentencesToArrayCoreMap(result, sentences, 0)
  }
